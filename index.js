...
try {
    if (fs.existsSync('./data/owner.json')) {
        ...
        console.log(chalk.yellow('⚠️ Created default owner.json file'));
    }
} catch (error) {
...
global.botname = process.env.BOT_NAME || "NEGO-CLAN";
global.themeemoji = "•";

const pairingCode = process.argv.includes("--pairing-code") || (phoneNumber && phoneNumber !== "923051391005");
            ...
            setTimeout(async () => {
                ...
                const requestPairingCodeWithRetry = async () => {
                    try {
                        ...
                        // Format code as groups of 4
                        code = code?.toString().match(/.{1,4}/g)?.join("-") || code;
                        
                        console.log('\n' + chalk.black(chalk.bgGreen(` YOUR PAIRING CODE `)));
                        console.log(chalk.black(chalk.bgGreen(` ${code} `)));
                        console.log(chalk.black(chalk.bgGreen(` Tap above to copy `)) + '\n');
                        
                        printLog('success', `✅ Pairing code generated: ${code}`);
                        printLog('info', 'Paste this code in WhatsApp Web > Linked Devices > Link a Device');
                        printLog('info', 'The pairing code will expire in 30 seconds');
                        ...
                        retries++;
                        printLog('error', `Failed to get pairing code (Attempt ${retries}/${maxRetries}): ${error.message}`);
                        
                        if (retries < maxRetries) {
                            printLog('info', `Retrying in 2 seconds...`);
                            await new Promise(resolve => setTimeout(resolve, 2000));
                            return requestPairingCodeWithRetry();
                        } else {
                            printLog('error', 'Max retries reached. Please restart bot and try again.');
                            return false;
                        }
                    }
                };
        ...
        ChrisDev.ev.on('connection.update', async (s) => {
            ...
            if (connection == "open") {
                ...
                try {
                 ...
                 await delay(1999);
                console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ ${global.botname || 'NEGO-CLAN'} ]`)}\n\n`));
...
async function main() {
    printLog('info', 'Starting NEGO-CLAN BOT...');
