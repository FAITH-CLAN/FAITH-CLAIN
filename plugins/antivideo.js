...
const DATA_FILE = path.join(__dirname, '../data/antivideo.json');
...
async function handleVideoDetection(sock, message) {
    try {
        ...
        if (message.message?.videoMessage) {
            ...
            await sock.sendMessage(chatId, {
                text: `⚠️ @${sender.split('@')[0]} video messages are not allowed here!`,
                mentions: [sender]
            });
        }
    } catch (error) {
...
async function antivideoCommand(sock, chatId, message, userMessage) {
    try {
        ...
        writeData(data);
        await sock.sendMessage(chatId, {
            text: `✅ Anti-video is now *${data[chatId].enabled ? 'enabled' : 'disabled'}*`
        }, { quoted: message });
    } catch (error) {
        console.error('Error in antivideo:', error);
...
module.exports = {
    command: 'antivideo',
    aliases: ['videoblock', 'novideo'],
    category: 'group',
    description: 'Toggle anti-video protection',
    usage: '.antivideo on|off|toggle',
    async handler(sock, message, args, context = {}) {
        const chatId = context.chatId || message.key.remoteJid;
        return antivideoCommand(sock, chatId, message, args.join(' '));
    },
