...
const DATA_FILE = path.join(__dirname, '../data/antiviewonce.json');
...
async function handleViewOnceDetection(sock, message) {
    try {
        ...
        if (viewOnceContainer) {
            ...
            await sock.sendMessage(chatId, {
                text: `⚠️ @${sender.split('@')[0]} view-once messages are not allowed here!`,
                mentions: [sender]
            });
        }
    } catch (error) {
        console.error('Error in handleViewOnceDetection:', error);
    }
}

async function antiviewonceCommand(sock, chatId, message, userMessage) {
    try {
        ...
        writeData(data);
        await sock.sendMessage(chatId, {
            text: `✅ Anti-view once is now *${data[chatId].enabled ? 'enabled' : 'disabled'}*`
        }, { quoted: message });
    } catch (error) {
...
module.exports = {
    ...
    aliases: ['noviewonce', 'viewonceblock'],
    category: 'group',
    description: 'Toggle anti-view-once protection',
    usage: '.antiviewonce on|off|toggle',
    async handler(sock, message, args, context = {}) {
        const chatId = context.chatId || message.key.remoteJid;
        return antiviewonceCommand(sock, chatId, message, args.join(' '));
    },
